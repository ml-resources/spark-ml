...
/**
  * @author Rajdeep Dua
  */
object MovieLensDataGausianMixtureClustering {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val userClusterModel = gmmKMeans.run(userVectors)

    for (i <- 0 until movieClusterModel.k) {
      println("weight=%f\nmu=%s\nsigma=\n%s\n" format
        (movieClusterModel.weights(i), movieClusterModel.gaussians(i).mu, movieClusterModel.gaussians(i).sigma))
    }
    println("done")
